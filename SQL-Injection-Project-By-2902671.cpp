...
bool isSQLInjection(const std::string& query) {
    std::regex pattern(R"(\bor\b\s+['"]?\w+['"]?\s*=\s*['"]?\w+['"]?)", std::regex::icase);
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Detect SQL injection attempt (generic OR value=value pattern)
    if (isSQLInjection(sql)) {
        std::cout << "SQL Injection Detected! Query Blocked." << std::endl;
        return false;
    }
